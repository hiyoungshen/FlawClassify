	...
	def train(self,mode):
		...
		with self.__sess.as_default():
			...
			for i in range(self.model.step, self.__Param["epochs_num"] + self.model.step):
				...
				self.logger.info('iter: {} / 1000, train_mode:{}, total loss: {}'.format(self.model.step, mode,iter_loss))
				print('iter: {} / 1000, train_mode:{}, total loss: {}\n'.format(self.model.step, mode,iter_loss))
				#保存模型
				if i % self.__Param["save_frequency"] == 0 or i==self.__Param["epochs_num"] + self.model.step-1:
					with tf.name_scope('loss'):
						...
						lossvalue = tf.Summary(value=[tf.Summary.Value(tag="loss", simple_value=0)])
						writer = tf.summary.FileWriter("logb/", self.__sess.graph)
						writer.add_summary(lossvalue, i)
					self.model.save()
				# if i % self.__Param["valid_frequency"] == 0 and i>0:
				# 	self.valid()
				self.model.step += 1
		...
		with self.__sess.as_default():
			...
			for index in range(2):
				for batch in range(DataManager[index].number_batch):
					...
					mask_batch ,output_batch= self.__sess.run([self.model.mask,self.model.output_class],    feed_dict={self.model.Image: img_batch,})
					#mask3 = utils.concatImage([mask2, mask2])
					#mask3.save('./visualization/test\\kos31_exp6_num_249821.jpg')
					self.visualization(img_batch, label_pixel_batch,mask_batch, file_name_batch,save_dir=visualization_dir)
					mask2 = np.array(mask_batch[0]).squeeze(2)
