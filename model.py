...
#                     conv1 = slim.conv2d(input, 32, [5, 5], scope='conv1')
#                     conv2 = slim.conv2d(conv1, 32, [5, 5], scope='conv2')
#                     pool1 = slim.max_pool2d(conv2, [2, 2], [2, 2], scope='pool1')
#
#                     conv3 = slim.conv2d(pool1, 64, [5, 5], scope='conv3')
#                     conv4 = slim.conv2d(conv3, 64, [5, 5], scope='conv4')
#                     pool2 = slim.max_pool2d(conv4, [2, 2], [2, 2], scope='pool2')
#
#                     conv5 = slim.conv2d(pool2, 64, [5, 5], scope='conv5')
#                     conv6 = slim.conv2d(conv5, 64, [5, 5], scope='conv6')
#                     pool3 = slim.max_pool2d(conv6, [2, 2], [2, 2], scope='pool3')
#
#                     conv7 = slim.conv2d(pool3, 1024, [15, 15], scope='conv7')
#
#                     conv_t1 = slim.conv2d_transpose(conv7, 64, kernel_size= [3,3],stride=[2,2], scope='conv_t1')
#                     merge1 = tf.concat([conv_t1, conv6], 3)
#                     conv8 = slim.conv2d(merge1, 64, [5, 5], scope='conv8')
#                     conv9 = slim.conv2d(conv8, 64, [5, 5], scope='conv9')
#
#                     conv_t2 = slim.conv2d_transpose(conv9, 64, kernel_size= [3,3],stride=[2,2], scope='conv_t2')
#                     merge2 = tf.concat([conv_t2, conv4], 3)
#                     conv10 = slim.conv2d(merge2, 64, [5, 5], scope='conv10')
#                     conv11 = slim.conv2d(conv10, 64, [5, 5], scope='conv11')
#
#                     conv_t3 = slim.conv2d_transpose(conv11, 32,kernel_size= [3,3],stride=[2,2], scope='conv_t3')
#                     merge3 = tf.concat([conv_t3, conv2], 3)
#                     conv12 = slim.conv2d(merge3, 32, [5, 5], scope='conv12')
#                     net = slim.conv2d(conv12, 32, [5, 5], scope='conv13')
#
#                     features = net
#                     net = slim.conv2d(net, 1, [1, 1], activation_fn=None, scope='conv14')
#                     logits_pixel = net
...
#         PixelLabel = tf.placeholder(tf.float32, shape=(self.__batch_size, IMAGE_SIZE[0] , IMAGE_SIZE[1] , 1),
#                                     name='PixelLabel')
...
#             tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_pixel, labels=PixelLabel_reshape))
        ...
        def SegmentNet(input, scope, is_training, reuse=None):
            with tf.variable_scope(scope, reuse=reuse):
                with slim.arg_scope([slim.conv2d],
                                    ...
                                    activation_fn=tf.nn.relu,
                                    normalizer_fn=slim.batch_norm):
                    conv1 = slim.conv2d(input, 64, [3, 3], scope='conv1')
                    conv2 = slim.conv2d(conv1, 64, [3, 3], scope='conv2')
                    pool1 = slim.max_pool2d(conv2, [2, 2], [2, 2], scope='pool1')

                    conv3 = slim.conv2d(pool1, 128, [3, 3], scope='conv3')
                    conv4 = slim.conv2d(conv3, 128, [3, 3], scope='conv4')
                    pool2 = slim.max_pool2d(conv4, [2, 2], [2, 2], scope='pool2')

                    conv5 = slim.conv2d(pool2, 256, [3, 3], scope='conv5')
                    conv6 = slim.conv2d(conv5, 256, [3, 3], scope='conv6')
                    pool3 = slim.max_pool2d(conv6, [2, 2], [2, 2], scope='pool3')

                    conv7 = slim.conv2d(pool3, 512, [3, 3], scope='conv7')
                    conv8 = slim.conv2d(conv7, 512, [3, 3], scope='conv8')
                    pool4 = slim.max_pool2d(conv8, [2, 2], [2, 2], scope='pool4')

                    conv9 = slim.conv2d(pool4, 1024, [3, 3], scope='conv9')

                    conv_t1 = slim.conv2d_transpose(conv9, 512, kernel_size= [3,3],stride=[2,2], scope='conv_t1')
                    merge1 = tf.concat([conv_t1, conv8], 3)
                    conv10 = slim.conv2d(merge1, 512, [3, 3], scope='conv10')
                    conv11 = slim.conv2d(conv10, 512, [3, 3], scope='conv11')

                    conv_t2 = slim.conv2d_transpose(conv11, 256, kernel_size= [3,3],stride=[2,2], scope='conv_t2')
                    merge2 = tf.concat([conv_t2, conv6], 3)
                    conv12 = slim.conv2d(merge2, 256, [3, 3], scope='conv12')
                    conv13 = slim.conv2d(conv12, 256, [3, 3], scope='conv13')

                    conv_t3 = slim.conv2d_transpose(conv13, 128,kernel_size= [3,3],stride=[2,2], scope='conv_t3')
                    merge3 = tf.concat([conv_t3, conv4], 3)
                    conv14 = slim.conv2d(merge3, 128, [3, 3], scope='conv14')
                    conv15 = slim.conv2d(conv14, 128, [3, 3], scope='conv15')

                    conv_t4 = slim.conv2d_transpose(conv15, 64,kernel_size= [3,3],stride=[2,2], scope='conv_t4')
                    merge4 = tf.concat([conv_t4, conv2], 3)
                    conv16 = slim.conv2d(merge4, 64, [3, 3], scope='conv16')
                    conv17 = slim.conv2d(conv16, 64, [3, 3], scope='conv17')

                    features = conv17

                    net = slim.conv2d(conv17, 1, [1, 1], activation_fn=None, scope='conv18')
