...
DefaultParam = {
    "mode": "training",  # 模式  {"training","testing","savePb" }
    "train_mode":"segment", #训练模式，{"segment":only train segment net,"decision": only train decision net, "total": both}
    "epochs_num": 1000,
    "batch_size": 1,
    "learn_rate": 0.001,
    "momentum": 0.9,                 # 优化器参数(disable)
    "data_dir": "./dataset",  # 数据路径
    "checkPoint_dir": "checkpoint5",  # 模型保存路径
    "Log_dir": "Log",  # 日志打印路径
    "valid_ratio": 0.2,  # 数据集中用来验证的比例  (disable)
    "valid_frequency": 10,  # 每几个周期验证一次  (disable)
    "save_frequency": 50,  # 几个周期保存一次模型
    "max_to_keep": 5,  # 最多保存几个模型
    "b_restore": False,  # 导入参数
